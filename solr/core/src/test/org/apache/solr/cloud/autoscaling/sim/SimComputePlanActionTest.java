...
public class SimComputePlanActionTest extends SimSolrCloudTestCase {
  ...
  public void init() throws Exception {
    ...
    cluster.getDistribStateManager().setData(ZkStateReader.SOLR_AUTOSCALING_CONF_PATH, "{}".getBytes(UTF_8), -1);
    ...
    try {
      cluster.request(CollectionAdminRequest.deleteCollection("testNodeLost"));
    } catch (Exception e) {
      // expected if testNodeLost hasn't run already
    }
    try {
      cluster.request(CollectionAdminRequest.deleteCollection("testNodeAdded"));
    } catch (Exception e) {
      // expected if testNodeAdded hasn't run already
    }
    try {
      cluster.request(CollectionAdminRequest.deleteCollection("testNodeWithMultipleReplicasLost"));
    } catch (Exception e) {
  ...
  public void testNodeLost() throws Exception  {
    ...
    String setTriggerCommand = "{" +
        ...
        "{'name':'test','class':'" + SimComputePlanActionTest.AssertingTriggerAction.class.getName() + "'}]" +
        "}}";

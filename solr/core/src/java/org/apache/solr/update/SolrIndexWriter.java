  ...
  private SolrIndexWriter(SolrCore core, String name, String path, Directory directory, boolean create, IndexSchema schema, SolrIndexConfig config, IndexDeletionPolicy delPolicy, Codec codec) throws IOException {
    ...
    if (config.metricsInfo != null && config.metricsInfo.initArgs != null) {
      ...
      if (mergeDetails) {
        ...
        majorMergedDocs = metricManager.meter(registry, "docs", SolrInfoBean.Category.INDEX.toString(), "merge", "major");
        majorDeletedDocs = metricManager.meter(registry, "deletedDocs", SolrInfoBean.Category.INDEX.toString(), "merge", "major");
      }
      if (mergeTotals) {
        minorMerge = metricManager.timer(registry, "minor", SolrInfoBean.Category.INDEX.toString(), "merge");
        majorMerge = metricManager.timer(registry, "major", SolrInfoBean.Category.INDEX.toString(), "merge");
        mergeErrors = metricManager.counter(registry, "errors", SolrInfoBean.Category.INDEX.toString(), "merge");
        metricManager.registerGauge(registry, () -> runningMajorMerges.get(), true, "running", SolrInfoBean.Category.INDEX.toString(), "merge", "major");
        metricManager.registerGauge(registry, () -> runningMinorMerges.get(), true, "running", SolrInfoBean.Category.INDEX.toString(), "merge", "minor");
        metricManager.registerGauge(registry, () -> runningMajorMergesDocs.get(), true, "running.docs", SolrInfoBean.Category.INDEX.toString(), "merge", "major");
        metricManager.registerGauge(registry, () -> runningMinorMergesDocs.get(), true, "running.docs", SolrInfoBean.Category.INDEX.toString(), "merge", "minor");
        metricManager.registerGauge(registry, () -> runningMajorMergesSegments.get(), true, "running.segments", SolrInfoBean.Category.INDEX.toString(), "merge", "major");
        metricManager.registerGauge(registry, () -> runningMinorMergesSegments.get(), true, "running.segments", SolrInfoBean.Category.INDEX.toString(), "merge", "minor");
        flushMeter = metricManager.meter(registry, "flush", SolrInfoBean.Category.INDEX.toString());
      }
    }
  }

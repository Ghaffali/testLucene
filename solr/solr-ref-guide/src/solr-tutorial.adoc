...
It is organized into three sections that each build on the one before it. The first exercise will ask you to start Solr, create a collection, index some basic documents, and then perform a few searches.
...
This will start an interactive session that will start two Solr "servers" on your machine. This command has an option to run without prompting you for input (`-noprompt`), but we want to modify two of the defaults so we will not use that option in this example.
...
Documents Screen::
Use the Admin UI <<documents-screen.adoc#documents-screen,Documents tab>> (at http://localhost:8983/solr/#/gettingstarted/documents) to paste in a document to be indexed, or select `Document Builder` from the `Document Type` dropdown to build a document one field at a time. Click on the kbd:[Submit Document] button below the form to index your document.
...
This Guide will be your best resource for learning more about Solr. But Solr also has a robust community. For more information, check out the Solr website's http://lucene.apache.org/solr/resources.html[Resources page].

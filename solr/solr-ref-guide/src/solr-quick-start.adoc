...
== Exercise 1: Launch Solr and Index Techproducts Example Data

=== Launch Solr in SolrCloud Mode
To launch Solr, run: `bin/solr start -e cloud`.

This will start an interactive session that will start two Solr "servers" on your machine. This command has an option to run without prompting you for input (`-noprompt`), but we want to modify one of the defaults so we will not use that option in this example.
...
----
The default in this case is "2", which is what we want for this example, so you can simply press kbd:[enter].
...
----
This will be the port that one of our nodes runs on. Unless you know you have something else running on port 8983 on your machine, accept this default option also by pressing kbd:[enter]. If something is already using that port, you will be asked to choose another port.
...
Notice that it has started two instances of Solr on two nodes. It prints the commands it uses for your reference. Because we are starting in SolrCloud mode, and did not define any details about an external ZooKeeper cluster, Solr launches its own ZooKeeper and connects both nodes to it.
...
----
Here's the first place where we'll deviate from the default options. This tutorial will ask you to index some sample data included with Solr, called the "techproducts" data. Let's name our collection that so it's easy to differentiate from other collections we'll create later. Enter `techproducts` at the prompt and hit kbd:[enter].

[source,subs="verbatim,attributes+"]
----
How many shards would you like to split techproducts into? [2]
----
This is asking how you want to split your index across the two nodes. Choosing two means we will split the index relatively evenly across both nodes, which is a good way to start. Accept the default by hitting kbd:[enter].

[source,subs="verbatim,attributes+"]
----
How many replicas per shard would you like to create? [2]
----
A replica is a copy of the index that's used for failover. Two is fine to start with here also, so accept the default again by hitting kbd:[enter].
...
The data we will index is in the `example/exampledocs` directory. The documents are in a mix of document formats, but fortunately we can index them all at once:
...
.Query Screen
image::images/solr-quick-start/quickstart-query-screen.png[Solr Quick Start: techproducts Query screen with results]
...
What's happening here is that we are using Solr's query parameter (`q`) with a special syntax that requests all documents in the index (`\*:*`). We don't get all the documents, however, because of the default for a parameter called `rows`, which you can see in the form is `10`. You can change the parameter in the UI or in the defaults if you wish.
...
To search for a term, enter it as the `q` param value in box in the Solr Admin UI Query screen, replacing `\*:*` with the term you want to find. Enter "foundation" and hit btn:[Execute Query] again.
...
The response indicates that there are 4 hits (`"numFound":4`). We've only included one document here, but since 4 hits is lower than the `rows` parameter default of 10 to be returned, you should see all of them.
...
To search for a multi-term phrase, enclose it in double quotes: `q="multiple terms here"`. For example, to search for "CAS latency" by entering that phrase in quotes to the `q` box in the Admin UI.
...
`bin/solr create -c <yourCollection>`
...
== Exercise 2: Modify Your Schema and Index Films Data

This section of the tutorial will build on the last one and introduce you to the index schema and Solr's powerful faceting features.

=== Restart Solr

Did you stop Solr after the last exercise? No? Then go ahead.
...
One reason for this is we're going to use a feature in Solr called "field guessing", where Solr makes a best attempt at guessing what type of data is in a field while it's indexing it. It also creates new fields in the schema for new fields that it finds. This mode is called "Schemaless". We'll see the benefits and limitations of this approach to help you decide how and where to use it in your real application.
...
When you initially started Solr in the first exercise, you had a choice of a ConfigSet to use. The one you chose had a schema that was pre-defined for the data you later indexed. This time, you're going to use a ConfigSet that has a very minimal schema and let Solr figure out from our data what fields to add.

The data you're going to index is related to films, so start by creating a collection named "films" that uses the `_default` ConfigSet:

`bin/solr create -c films -s 2 -rf 2`

Whoa, wait. You didn't specify a ConfigSet! That's fine, the `_default` is appropriately named, since it's the default and is used if you don't specify one at all.

You did, however, set two parameters `-s` and `-rf`. Those are the number of shards to split the collection across (2) and how many replicas to create (2). This is the same as the options we chose with the interactive example from the first exercise.
...
==== Preparing Schemaless for Data

There are two parallel things happening with our schema.

First, we are using a "managed schema", which is configured to only be modified by Solr's Schema API. That means we should not hand-edit it. Solr's Schema API allows us to make changes to fields, field types, and other types of schema rules.

Second, we are using "field guessing", which is configured in the `solrconfig.xml` file, which has most of Solr's various configuration settings. This field guessing is designed to allow us to start using Solr without having to define all the fields we think will be in our documents before trying to index them. This is why we call it "schemaless", because you can start quickly and let Solr create fields for you as it encounters fields in documents.

Sounds great! Well, not really, there are limitations. It's a bit brute force still, and if it guesses wrong, you can't change much about a field after data has been indexed without having to reindex it. If we only have a few thousand documents that might not be bad, but if you have millions and millions of documents or, worse, don't have access to the original data anymore, this can be a real problem.

For these reasons, the Solr community does not recommend going to production without a schema that you have defined yourself. By this we mean that the schemaless features are fine to start with, but you should still always make sure your schema matches your expectations for how you want your data indexed and how users are going to try to query it.
...
The films data we are going to index has a small number of fields: an ID, director names, film name, release date, and genres. If you look at one of the files in `example/films`, you'll see the first film is named _.45_, released in 2006. As the first record, Solr is going to guess the field type based on the data in the record. If we go ahead and index this data, that first film is going to indicate to Solr that that field type is a "float" numeric field, and will create a "name" field with a type `FloatPointField`. All data after this record will be expected to be a float.

Well, that's not going to work. We have titles like _A Mighty Wind_ and _Chicken Run_, which are strings - decidedly not numeric and not floats. What will happen is later titles will cause an error and indexing will fail. That's not a great way to start.

What we can do is set up the "name" field in Solr before we index the data to be sure that it always interprets it as a string. At the command line, enter this curl command:
...
The configuration we're using now is missing part of that configuration. We would need to define a field to search on for every one of our queries. We can, however, set up what we call a catchall field by defining a copy field that will take all data from all fields and index it into a field named `\_text_`. Let's do that now.
...
TIP: It can be very expensive to do this with your production data because it tell Solr to effectively index everything twice. It will make indexing slower, and also make your index larger. With your production data, you will want to be sure you only copy fields that really warrant it for your application.

OK, now we're ready to index the data and start playing around with it.

=== Index Sample Film Data

The films data we will index is located in the `example/films` directory of your installation. It comes in three formats: JSON, XML and CSV. Pick one of the formats and index it into the "films" collection:
...
If you go to the Query screen in the Admin UI for films (http://localhost:8983/solr/#/films/query) and hit kbd:[Execute Query] you should see 1100 results, with the first 10 returned to the screen.

Let's do a query to see if the "catchall" field worked properly. Enter "comedy" in the `q` box and hit kbd:[Execute Query] again. You should see get 417 results. Feel free to play around with other searches before we move on to faceting.
...
`curl '\http://localhost:8983/solr/gettingstarted/select?q=*:*&rows=0&facet=true&facet.field=genre_str'`
...
Like our previous exercise, this data may not be relevant to your needs. We can clean up our work by deleting the collection:

`bin/solr delete -c films`

== Exercise 3: Index Your Own Data

For this last exercise, you will work with your own data. This can be files on your local hard drive, a set of data you have worked with before, or maybe a sample of the data you intend to index to Solr for your production application. This exercise is intended to get you thinking about what you will need to do for your application:
...
Before you get started, create a new collection, named whatever you'd like. In this example, the collection will be named "localDocs"; replace that value with whatever name you choose.

`./bin/solr create -c localDocs -s 2 -rf 2`

Again, as we saw from exercise 2 above, this will use the `_default` ConfigSet and all the schemaless features it provides. This may cause problems with our data.
...
In this example, assume there is a directory named "Documents" locally. To index it, we would issue a command like this:

`./bin/post -c localDocs ~/Documents`

You may get errors as it works through your documents. These might be caused by the field guessing, or the file type may not be supported. Indexing content such as this demonstrates the need to plan Solr for your data, which requires understanding it and perhaps also some trial & error.
...
SolrJ::
Use <<using-solrj.adoc#using-solrj,SolrJ>> for JVM-based languages or other <<client-apis.adoc#client-apis,Solr clients>> to programmatically create documents to send to Solr.
...
To delete all documents, you can use a command like:
...
Solr also has a robust community. For more information, check out the following resources:

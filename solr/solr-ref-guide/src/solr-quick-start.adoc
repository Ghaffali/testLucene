...
You can search for "solr" by loading the Admin UI http://localhost:8983/solr/#/gettingstarted/query[Query tab], enter "solr" in the `q` param (replacing `*:*`, which matches all documents), and "Execute Query". See the <<searching,Searching>> section below for more information.
...
To flatten (and/or split) and index arbitrary structured JSON, a topic beyond this quick start guide, check out <<uploading-data-with-index-handlers.adoc#UploadingDatawithIndexHandlers-TransformingandIndexingCustomJSON,Transforming and Indexing Custom JSON data>>.
...
* Import records from a database using the <<uploading-structured-data-store-data-with-the-data-import-handler.adoc#uuploading-structured-data-store-data-with-the-data-import-handler,Data Import Handler (DIH)>>.

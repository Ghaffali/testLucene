...
`Example 4`::
Place all replicas on a node with system property region=us-east-1. Note that we have to write this rule in the negative sense i.e. *0* replicas must be on nodes *not* having the sysprop `region=us-east-1`
[source,json]
----
{"replica": 0, "sysprop.region": "!us-east-1"}
----
